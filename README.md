...
> **Note**: 이 섹션은 Fork에서 추가된 설정 가이드입니다. 원본 PaperBanana README는 [아래](#overview-of-paperbanana)에서 시작됩니다.
...
3. **크레딧 사용량 및 결제 확인** — Chrome에서 [console.cloud.google.com](https://console.cloud.google.com) 접속 후:
   - 상단 프로젝트 선택 드롭다운에서 사용 중인 프로젝트를 선택합니다.
   - 좌측 메뉴 **"결제"** (Billing) 클릭 → **"개요"** (Overview) 에서 현재 잔여 크레딧과 사용량을 확인합니다.
   - **"보고서"** (Reports) 탭에서 날짜별/서비스별 상세 비용을 확인할 수 있습니다.
   - **"예산 및 알림"** (Budgets & alerts) 에서 예산 한도를 설정하면 과금 시 이메일 알림을 받을 수 있습니다.
   - 무료 크레딧 잔액은 "크레딧" (Credits) 탭 또는 결제 개요 페이지 상단에서 확인 가능합니다.
4. **gcloud CLI 설치 및 인증**:
   ...
   **Windows:**
   - [GoogleCloudSDKInstaller.exe](https://cloud.google.com/sdk/docs/install#windows) 다운로드 후 기본 옵션으로 설치합니다.
   - **중요**: 설치 후 시작 메뉴에서 **"Google Cloud SDK Shell"** 을 실행하세요. 일반 cmd/PowerShell에서는 PATH가 적용되지 않아 오류가 발생할 수 있습니다.
   ```cmd
   gcloud auth login
   gcloud auth application-default login
   ...
   ```
   - ADC 인증 파일 저장 위치: `C:\Users\<USERNAME>\AppData\Roaming\gcloud\application_default_credentials.json`
...
# Default Models
defaults:
  model_name: "gemini-3.1-pro-preview"                       # 텍스트 생성용 (default)
  image_model_name: "gemini-3.1-flash-image-preview"         # 이미지 생성용 (default)
  #
  # 다른 모델 옵션 (주석 해제하여 사용):
  # --- 텍스트 생성 ---
...
**Windows (Google Cloud SDK Shell 또는 cmd):**
```cmd
:: 1. 환경변수 설정
...
:: configs\model_config.yaml 편집 (메모장 등)
...
> **정상 실행 시** 터미널에 `Initialized Gemini Client with Vertex AI (project: ...)` 메시지가 출력됩니다.
...
| 증상 | 해결 |
|------|------|
| `Permission denied: adc.json` (Windows) | 일반 cmd 대신 **Google Cloud SDK Shell** 에서 gcloud 명령어 실행 |
| `quota exceeded` 또는 `API not enabled` | `gcloud auth application-default set-quota-project YOUR_PROJECT_ID` 실행 |
| `streamlit: command not found` | 가상환경 활성화 확인 — Ubuntu: `source .venv/bin/activate`, Windows: `.venv\Scripts\activate` |
| `ModuleNotFoundError` | `uv pip install -r requirements.txt` 재실행 |
| `Warning: Could not initialize Anthropic/OpenAI Client` | Gemini만 사용하는 경우 정상 메시지. 무시 가능 |

---

<br/></br>

## Original PaperBanana README

> **Note**: 아래부터는 원본 PaperBanana 저장소의 README 내용입니다.
...
### Step3: Installing the Environment
...
### Step4: Downloading the Dataset (Optional)
The framework works without the dataset (Retriever Agent's few-shot learning will be bypassed).
For benchmark evaluation, download [PaperBananaBench](https://huggingface.co/datasets/dwzhu/PaperBananaBench):

```bash
# huggingface_hub is included in requirements.txt
python -c "
from huggingface_hub import snapshot_download
snapshot_download(
